...
export default function UserChip({data}) {
  ...
  const handleCloseDetails = () => {setOpenDetails(false);};
    ...
    useEffect(() => {   
      setImage(data.image)  ;
      setUserName(capitalizeFirstLetter(data.firstName)+" " +capitalizeFirstLetter(data.lastName));
      setUser(data)
    });
  return (
    <div>
                  ...
                  <div className="card mb-1">
                    <div className="pt-2 text-center">
                      {user.image && user.image!==null ? 
                          <img src={`${process.env.REACT_APP_BACKEND_URL}/uploads/userProfile/${user.image}`} alt="profile image"
                          className="rounded-circle img-fluid" style={{ width: '150px' , height: '150px' }} />
                          :
                          <img src={DefaultImage}
                            alt="profile ima" id="img" className="rounded-circle img-fluid" style={{ width: '150px', height: '150px' }} />
                      } 
